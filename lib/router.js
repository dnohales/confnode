Router.configure({
    ...
    waitOn: function() {
        return [Meteor.subscribe('rooms'), Meteor.subscribe('userData')];
    }
});
...
var editPermission = function() {
    ...
    if (room.creatorName === user.username) {
        this.next();
    } else {
        this.render('accessDenied');
    }
...
var roomGuestsAccess = function() {
    ...
    if (!room.public) {
        if (room.guests.indexOf(userEmail) !== -1 || room.creatorName === user.username) {
            this.next();
        } else {
